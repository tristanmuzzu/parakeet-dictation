...
You are setting up free, offline, hotkey-driven voice dictation on the user's **Windows** machine. Everything is local; the only download is the speech model from Hugging Face on first run. Target result: the user presses **Ctrl+Win**, speaks, presses **Ctrl+Win** again, and the recognized text is typed into the focused window.
...
1. Windows 10/11.
2. Python **3.10–3.12** available (`py -3.12 --version` or `python --version`). 3.13+ may lack prebuilt wheels for some dependencies — if only 3.13+ exists, install 3.12 first (winget: `winget install Python.Python.3.12`).
...
- If int8 weights fail to download it falls back to fp32 (~600 MB, slower load) — still fine.
...
The app writes `dictation.log` in the repo root — your primary verification surface:
...
Only one instance runs at a time (TCP lock on 127.0.0.1:49731); extra launches exit silently — check the log's `another instance already running` line if confused.
...
| Hotkey dead only after boot / model load | Keyboard hook killed by Windows when the hooked process pegs CPU (LowLevelHooksTimeout). Should not happen — model loads in a separate worker process by design. | Verify only ONE dictation instance runs and that you didn't merge worker into main. |
| `add_hotkey` style combos don't fire | The `keyboard` lib mishandles modifier-only combos (Ctrl+Win has no regular key), and `suppress=True` makes it eat keys globally without firing. | Don't refactor to `add_hotkey`; keep the raw `keyboard.hook` + manual state tracking. No suppression needed — only Win *alone* opens the Start menu. |
...
- Do not "fix" the two-process split by loading the model in the main process — that reintroduces the silent hook-death bug.
